class Movielist < Movie
    ...
    @filename = filename
    @movielist = CSV.read(@filename, col_sep:('|'), 
  ...
  def producer_stat
    @movielist.group_by(&:producer).
      each { |film, list| puts film + "     " + "#{list.size}" }
  end
  def actor_stat
    @movielist.map(&:actor).flatten.sort_by{ |actor| actor.split(' ').last }.reduce(Hash.new(0)) { |sum, i| sum[i] +=1; sum}
  end
  def month_stat
    @movielist.map( &:date ).reject { |month| month == nil }.sort_by { |month| month.to_s[5..7] }.
      group_by { |month| month.strftime("%B") }
  end
end
